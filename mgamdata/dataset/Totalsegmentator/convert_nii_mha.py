...
from mgamdata.dataset.Totalsegmentator.meta import CLASS_INDEX_MAP, generate_subset_class_map_and_label_map
...
def convert_one_case(args):
    series_input_folder, series_output_folder, spacing, size, subset = args
...
def convert_and_save_nii_to_mha(input_dir:str,
    ...
    for series_name in os.listdir(input_dir):
        if os.path.isdir(os.path.join(input_dir, series_name)):
            ...
            task_list.append((series_input_folder, output_dir, spacing, size, subset))
