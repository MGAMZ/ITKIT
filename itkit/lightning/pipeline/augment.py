...
class RandomPatch3D(BaseTransform):
    def __init__(self, patch_size: Sequence[int], keys: list[str] = ["image", "label"]):
...
class RandomRotate3D(mm_RandomRotate3D):
    def __init__(self, keys: list[str], interp_orders: list[int], *args, **kwargs):
        ...
        self.interp_orders = (
            interp_orders if isinstance(interp_orders, list) else [interp_orders]
        )
...
class AutoPad(BaseTransform):
    ...
    def __call__(self, sample: dict):
        ...
        if any(p[0] > 0 or p[1] > 0 for p in pad_params):
            ...
            if "label" in sample:
                label_do_not_have_channel_dimension = (
                    sample["image"].ndim - sample["label"].ndim
                ) == 1
                sample["label"] = np.pad(
                    sample["label"],
                    (
                        pad_params[1:]
                        if label_do_not_have_channel_dimension
                        else pad_params
                    ),
                    mode="constant",
