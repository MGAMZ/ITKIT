...
class SlideWindowConfig:
    ...
    argmax_batchsize: int | None = (
        None  # None means no batching, direct argmax on class dim.
    )


def ArgMaxBatchedCalculator(
    logits: Tensor,
    batch_size: int | None,
    accumulate_device: torch.device,
    calculate_device: torch.device,
) -> Tensor:
    ...
    # Batched argmax on last dim (e.g. for large 3D volumes)
        ...
        predictions = torch.empty(
            (N, *spatial_channels), dtype=torch.uint8, device=accumulate_device
        )
        ...
        for start in range(0, L, batch_size):
            ...
            predictions[..., start:end] = (
                ...
                .to(calculate_device)
                .argmax(dim=1)
                .to(dtype=torch.uint8, device=accumulate_device)
            )
...
class SegmentationBase(pl.LightningModule):
    ...
    def _logits_to_predictions(self, logits: Tensor) -> Tensor:
        if self.num_classes > 1:
            return ArgMaxBatchedCalculator(
                logits,
            ...
            assert (
                self.binary_segment_threshold is not None
            ), "Binary segmentation requires a threshold"
    ...
    def _parse_batch(
        ...
        if self.to_one_hot:
            label = (
                torch.nn.functional.one_hot(label.long(), num_classes=self.num_classes)
                .permute(0, 4, 1, 2, 3)
    ...
    def _compute_losses(self, logits: Tensor, targets: Tensor) -> dict[str, Tensor]:
        ...
        for i, criterion in enumerate(self.criteria):
            loss_name = (
                f"loss_{criterion.__class__.__name__}"
        ...
        if self.num_classes > 1:
            for class_idx, class_name in enumerate(self.class_names):
                ...
                metrics[f"Dice_{class_name}"] = (
                    2
                    * intersection
                    / (pred_mask.float().sum() + target_mask.float().sum() + self.eps)
                )
                metrics[f"Recall_{class_name}"] = intersection / (
                    target_mask.float().sum() + self.eps
                )
                metrics[f"Precision_{class_name}"] = intersection / (
                    pred_mask.float().sum() + self.eps
                )

            metrics[f"IoU_Avg"] = torch.mean(
                torch.stack(
                    [metrics[f"IoU_{class_name}"] for class_name in self.class_names]
                )
            )
            metrics[f"Dice_Avg"] = torch.mean(
                torch.stack(
                    [metrics[f"Dice_{class_name}"] for class_name in self.class_names]
                )
            )
            metrics[f"Recall_Avg"] = torch.mean(
                torch.stack(
                    [metrics[f"Recall_{class_name}"] for class_name in self.class_names]
                )
            )
            metrics[f"Precision_Avg"] = torch.mean(
                torch.stack(
                    [
            ...
            class_name = (
                self.class_names[0] if self.class_names is not None else "binary"
            )
    ...
    def validation_step(self, batch: dict[str, Any], batch_idx: int) -> dict[str, Any]:
        image, gt_segs = self._parse_batch(
            batch, self.slide_window_config.patch_accumulate_device
        )
    ...
    def predict_step(self, batch: dict[str, Any], batch_idx: int) -> dict[str, Any]:
        batch["predictions"] = self._logits_to_predictions(
            self.inference(batch["image"])
        )
        return batch
    ...
    def inference(self, inputs: Tensor) -> Tensor:
        if (
            self.slide_window_config.patch_size is not None
            and self.slide_window_config.patch_stride is not None
        ):
...
class Segmentation3D(SegmentationBase):
    def slide_inference(self, inputs: Tensor) -> Tensor:
        if (
            self.slide_window_config.patch_size is None
            or self.slide_window_config.patch_stride is None
            or len(self.slide_window_config.patch_size) != 3
            or len(self.slide_window_config.patch_stride) != 3
        ):
        ...
        with torch.no_grad():
            temp_input = inputs[
                :, :, : min(z_crop, z_img), : min(y_crop, y_img), : min(x_crop, x_img)
            ]
        ...
        for z_idx in range(z_grids):
            for y_idx in range(y_grids):
                for x_idx in range(x_grids):
                    ...
                    # Extract patch
                    patch = inputs[:, :, z1:z2, y1:y2, x1:x2].to(
                    ...
                    preds[:, :, z1:z2, y1:y2, x1:x2] += patch_cache.copy_(
                        crop_logits, non_blocking=True
                    )
...
@deprecated(
    "Seg3D_SlideWindowTrain includes complex manual optimization logic and is deprecated."
...
class Seg3D_SlideWindowTrain(Segmentation3D):
    def __init__(self, *args, **kwargs):
        ...
        self.automatic_optimization = (
            False  # manual backward and step is deployed in this mode.
        )
    ...
    def _parse_batch_slide_window(self, batch: dict[str, Any]) -> dict[str, Any]:
        assert (
            self.slide_window_config.patch_size is not None
        ), "Patch size must be set for sliding window training"
        assert (
            self.slide_window_config.patch_stride is not None
        ), "Patch stride must be set for sliding window training"

        image_cpu = batch["image"]
        label_cpu = batch[self.gt_sem_seg_key].half()  # (N, ...) without channel dim.
        if self.to_one_hot:
            classes = torch.arange(
                self.num_classes, device=label_cpu.device, dtype=label_cpu.dtype
            )
            shape = [1, self.num_classes] + [1] * (
                label_cpu.ndim - 1
            )  # [1, C, 1, 1, ...], will be auto broadcasted
        ...
        return {"image": image_cpu, "label": label_cpu, "patch_indices": indices}
        ...
        for b in range(batch_size):
            for z1, z2, y1, y2, x1, x2 in slide_window_indices:
                batch_patches.append(
                    image_cpu[b : b + 1, :, z1:z2, y1:y2, x1:x2].to(
                        self.device, non_blocking=True
                    )
                )
                batch_targets.append(
                    label_cpu[b : b + 1, ..., z1:z2, y1:y2, x1:x2].to(
                        self.device, non_blocking=True
                    )
                )
