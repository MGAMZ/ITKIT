...
class MemorySnapshotter:
    ...
    def take_snapshot(
        ...
        if snapshot["vram"].get("available", True):
            total_vram = sum(
                v.get("allocated_mb", 0)
                for k, v in snapshot["vram"].items()
                if k != "available"
            )
        ...
        if include_tensors:
            ...
            print(
                f"        ✓ Found {snapshot['tensors']['total_count']} tensors, "
    ...
    def _get_tensor_info(self) -> dict:
        ...
        for obj in all_objects:
            ...
            if scanned_count % 100000 == 0:
                print(
                    f"              ... scanned {scanned_count}/{len(all_objects)} objects, found {len(tensors)} tensors"
                )
    ...
    def _get_object_info(self) -> dict:
        ...
        for obj in all_objects:
            ...
            if analyzed_count % 100000 == 0:
                print(
                    f"              ... analyzed {analyzed_count}/{len(all_objects)} objects"
                )
        ...
        # 按数量排序
        top_by_count = sorted(type_counts.items(), key=lambda x: x[1], reverse=True)[
            :20
        ]
        ...
        return {
            ...
            "top_20_by_size_mb": [
                {"type": t, "size_mb": s / 1024 / 1024} for t, s in top_by_size
            ],
        }
    ...
    def _save_readable_report(self, snapshot: dict, filepath: str):
        ...
        with open(filepath, "w") as f:
            ...
            if vram.get("available", True):
                for device, info in vram.items():
                    if device != "available":
                        ...
                        f.write(
                            f"    Max Allocated: {info['max_allocated_mb']:.2f} MB\n"
                        )
            ...
            if "tensors" in snapshot:
                ...
                f.write(
                    f"  CUDA Tensors:     {tensors['cuda_count']} ({tensors['cuda_size_mb']:.2f} MB)\n"
                )
                f.write(
                    f"  CPU Tensors:      {tensors['cpu_count']} ({tensors['cpu_size_mb']:.2f} MB)\n\n"
                )

                f.write("  Top 10 Largest Tensors:\n")
                for i, tensor in enumerate(tensors["top_10_largest"], 1):
                    f.write(
                        f"    {i}. {tensor['shape']} {tensor['dtype']} on {tensor['device']}: "
