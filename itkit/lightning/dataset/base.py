    ...
    def transfer_batch_to_device(
        ...
        if self.override_transfer_batch_to_device is not None:
            return super().transfer_batch_to_device(
                batch, self.override_transfer_batch_to_device, dataloader_idx
            )
        else:
...
class UniversalDataModule(BaseDataModule):
    ...
    def setup(self, stage: Literal["fit", "validate", "test", "predict"]):
        ...
        val_end_idx = train_end_idx + max(
            1, int(len(self.dataset) * self.dataset.SPLIT_RATIO[1])
        )
...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        assert self.split is None, (
            "The dataset split for Lightning is implemented in DataModule, not in Dataset. "
