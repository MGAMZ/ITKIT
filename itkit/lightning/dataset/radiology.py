...
class MhaDataset(BaseDataset):
    ...
    def _split(self) -> list[str]:
        if not self.split_accordance.exists():
            raise FileNotFoundError(
                f"Split accordance directory not found: {self.split_accordance}"
            )
        if self.all_seriesUID is None:
            all_seriesUID = [file.stem for file in self.split_accordance.glob("*.mha")]
            all_seriesUID = sorted(
                all_seriesUID, key=lambda x: abs(int(re.search(r"\d+", x).group()))
            )
        ...
        split_id_val_test = int(
            len(self.all_seriesUID) * (self.SPLIT_RATIO[0] + self.SPLIT_RATIO[1])
        )
    ...
    def index_dataset(self):
        ...
        for avail_file_name in tqdm(
            available_file_names, f"Indexing Dataset | Split {self.split}"
        ):
...
@deprecated(
    "The patched dataset structure now align to common MhaDataset, so there're no need to use this class."
)
class MhaPatchedDataset(MhaDataset):
    def index_dataset(self):
        splited_series = set(self._split())
        existed_series = [
            f for f in os.listdir(self.image_root) if os.path.isdir(self.image_root / f)
        ]
        self.available_series = []
        for series in tqdm(
            splited_series.intersection(existed_series),
...
class LargeVolumeDataModule(BaseDataModule):
    ...
    def setup(self, stage: Literal["fit", "validate", "test", "predict"]):
        if stage == "fit":
            assert (
                self.patched_train_dataset is not None
            ), "Patched train dataset must be provided for training."
            self.train = self.patched_train_dataset.get_self_copy(stage)
        elif stage == "validate":
            assert (
                self.whole_volume_val_dataset is not None
            ), "Whole volume val dataset must be provided for validation."
            self.val = self.whole_volume_val_dataset.get_self_copy(stage)
        elif stage in ("test", "predict"):
            assert (
                self.whole_volume_test_dataset is not None
            ), "Whole volume test dataset must be provided for testing."
