...
CLASS_MERGE = {
    "general": {
        ...
        "subclavian_artery": ["subclavian_artery_left", "subclavian_artery_right"],
        "vertebrae": [
            "vertebrae_C1",
            "vertebrae_C2",
            "vertebrae_C3",
...
def generate_subset_class_map_and_label_map(subset_name: str):
    ...
    if subset_name not in SUBSETS:
        raise ValueError(
            f"Subset '{subset_name}' not found in SUBSETS. Available subsets: {list(SUBSETS.keys())}"
        )
    ...
    # 创建子集类名到新索引的映射
    subset_class_map = {
        class_name: idx for idx, class_name in enumerate(subset_classes)
    }
