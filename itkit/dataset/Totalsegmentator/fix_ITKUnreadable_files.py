...
def main():
    ...
    parser.add_argument(
        "--mp", type=int, default=0, help="使用的进程数量，0 表示不使用多进程"
    )
    ...
    if args.mp > 0:
        ...
        with mp.Pool(processes=num_processes) as pool:
            process_func = functools.partial(
                process_file, input_root=input_root, output_root=output_root
            )
            results = list(
                tqdm(
                    pool.imap_unordered(process_func, nifti_files),
                    total=file_count,
                    desc="处理文件",
                    dynamic_ncols=True,
                )
        ...
        print("使用单进程处理")
        success_count = 0
        for file_path in tqdm(nifti_files, desc="处理文件", dynamic_ncols=True):
