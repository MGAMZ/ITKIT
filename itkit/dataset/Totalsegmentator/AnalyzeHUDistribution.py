...
class meta_file_handler:
    ...
    def _init_dfs(self):
        ...
        self.per_class_dfs = {
            class_name: df_template.copy(deep=True)
    ...
    def register(self, class_name, series_id, **kwargs):
        if class_name == "whole":
            for feature_name, value in kwargs.items():
                self.whole_df.loc[
                    self.whole_df["image_id"] == series_id, feature_name
                ] = value

        elif class_name in CLASS_INDEX_MAP.keys():
            for feature_name, value in kwargs.items():
                self.per_class_dfs[class_name].loc[
                    self.per_class_dfs[class_name]["image_id"] == series_id,
                    feature_name,
                ] = value
                if feature_name == "mean":
                    self.whole_df.loc[
                        self.whole_df["image_id"] == series_id, "mean_" + class_name
                    ] = value
    ...
    def save(self):
        ...
        for class_name, df in self.per_class_dfs.items():
            class_distribution_path = os.path.join(
                save_folder, f"{class_name}_distribution.csv"
            )
...
def parse_all_cases(data_root: str, meta_csv_path: str, mp: bool = False):
    cases = []
    task_list = [
        os.path.join(data_root, case_folder)
    ...
    if mp:
        ...
        for case_folder in tqdm(task_list, leave=False, dynamic_ncols=True):
...
if __name__ == "__main__":
    parse_all_cases(data_root="", meta_csv_path="", mp=True)
