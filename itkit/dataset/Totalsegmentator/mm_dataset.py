...
class TotalsegmentatorIndexer:
    ...
    def _index(self, image_root: str, split: str):
        ...
        for series in tqdm(
            iterable=os.listdir(split_folder),
...
class Tsd_base:
    ...
    def __init__(
        self,
        meta_csv: str | None,
        class_reduction: dict | None = None,
        subset: str | None = None,
        **kwargs,
    ) -> None:
        if class_reduction is not None and subset is not None:
            raise ValueError(
                "Cannot specify both class_reduction and subset. Please use only one."
            )
        self.meta_table = pd.read_csv(meta_csv) if meta_csv is not None else None

        if class_reduction is not None:
            new_class_index_map, label_map = generate_reduced_class_map_and_label_map(
                class_reduction
            )
            Tsd_base.METAINFO = dict(classes=list(new_class_index_map.keys()))
            self.label_map = label_map
        elif subset is not None:
            subset_class_map, label_map = generate_subset_class_map_and_label_map(
                subset
            )
