...
def parse_one_case_segmentations(
    ...
    if subset is not None:
        ...
        for class_name in sorted(
            CLASS_INDEX_MAP.keys(), key=lambda x: CLASS_INDEX_MAP[x]
        ):
        ...
        idx_sorted_paths = sorted(
            all_nii_files, key=lambda x: CLASS_INDEX_MAP[os.path.basename(x)[:-7]]
        )
...
def convert_one_case(args):
    ...
    merged_itk = parse_one_case_segmentations(
        input_image_mha, series_input_folder, subset=subset, merge_rule=merge_rule
    )
