...
class TCGA_MetaParser:
    ...
    def get_series_uids(self, attr: str, val) -> str:
        ...
        if len(result) == 0 or result[0] > 1:
            print(
                Fore.YELLOW, f"Series UID not found for {attr}={val}", Style.RESET_ALL
            )
        return result[0]
...
class TCGA_Formatter(StandardFileFormatter):
    ...
    def _series_id(self, image_path: str, label_path: str | None) -> str:
        folder_rel_path = os.path.relpath(
            image_path, os.path.dirname(image_path)
        ).replace("\\", "/")
        return self.TCGA_meta.get_series_uids(
            TCGA_MetaParser.META_PATH_ATTR, folder_rel_path
        )
    ...
    def tasks(self) -> list:
        ...
        for seriesUID, dcms_folder in tqdm(samples, desc="Searching"):
            dcms_folder = os.path.join(
                self.args.data_root, str(dcms_folder).replace("\\", "/")
            )
            ...
            if len(dcm_files) >= self.MINIMUM_DCM_SLICES:
                ...
                task_list.append(
                    (
                        dcm_files[
                            0
                        ],  # Need only one sample, the loader will automatically determine all slices.
                        label_path,
                        self.args.dest_root,
                        seriesUID,
                        self.args.spacing,
