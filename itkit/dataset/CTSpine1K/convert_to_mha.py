...
def process_item(task):
    ...
    """
    data_file_path, label_file_path, base_name, output_image_path, output_label_path = (
        task
    )
...
def convert_ctspine_data(input_dir, out_dir, use_mp=False):
    ...
    for d_sub in sorted(data_subfolders):
        ...
        data_files = [f for f in os.listdir(d_sub) if f.lower().endswith(".nii.gz")]
...
def main():
    ...
    parser.add_argument(
        "out_dir", type=str, help="输出目录，将放置转换后的image和label文件夹"
    )
