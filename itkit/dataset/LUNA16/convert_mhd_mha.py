...
class LUNA16_formatter(StandardFileFormatter):
    def tasks(self) -> list:
        ...
        labels_list = [
            file for file in os.listdir(labels_folder) if file.endswith(".mhd")
        ]
        subset_image_folder_list = ["subset{}".format(i) for i in range(10)]
        images_list = [
            os.path.join(self.args.data_root, subset_image_folder, file)
            for subset_image_folder in subset_image_folder_list
            for file in os.listdir(
                os.path.join(self.args.data_root, subset_image_folder)
            )
