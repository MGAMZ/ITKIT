...
class mgam_MONAI_Patched_Structure(mgam_SemiSup_3D_Mha):
    ...
    def load_data_list(self) -> list[dict]:
        ...
        # Volume loading transforms (no cropping)
        self.volume_transforms = monai_transforms.Compose([
            monai_transforms.LoadImaged(keys=["image", "label"], allow_missing_keys=True),
            monai_transforms.EnsureChannelFirstd(keys=["image", "label"], allow_missing_keys=True),
