...
class SA_Med2D:
    ...
    IMAGE_NAME_DEFINITION = [
        "modality_sub-modality",
        "dataset name",
        "ori name",
        "slice_direction",
        "slice_index",
    ...
    def analyze_file_name(cls, file_name: str) -> Dict[str, str]:
        ...
        if len(part) == 4:
            ...
            raise ValueError(
                f"Found {len(part)} name parts in file name {file_name}, expect{len(cls.IMAGE_NAME_DEFINITION)} or {len(cls.MASK_NAME_DEFINITION)}"
            )

        analyze_result = {"type": file_type}
        for name, value in zip(
            (
                cls.IMAGE_NAME_DEFINITION
                if file_type == "image"
                else cls.MASK_NAME_DEFINITION
            ),
            part,
        ):
...
class DataStructurer(SA_Med2D):
                ...
                json.dump(
                    ...
                    open(
                        os.path.join(
                            ...
                            modality,
                            dataset_name,
                            f"{dataset_name}_img_mask_map.json",
                        ),
                        "w",
                    ),
                    indent=4,
    ...
    def Split_ClassLabelMap_into_ModalitySeperated(self):
        ClassLabelMapJSONPath = os.path.join(
            self.src_root, "SAMed2D_v1_class_mapping_id.json"
        )
        source_map = orjson.loads(open(ClassLabelMapJSONPath, "r").read())

        for modality in os.listdir(self.ModalityRootAfterInit):
            if os.path.isfile(os.path.join(self.ModalityRootAfterInit, modality)):
                continue
            modality_class_map = {}
            for dataset in os.listdir(
                os.path.join(self.ModalityRootAfterInit, modality)
            ):
                if os.path.isfile(
                    os.path.join(self.ModalityRootAfterInit, modality, dataset)
                ):
                    continue
    ...
    def Filter_ClassLabelMap_to_Truely_Existed_with_ModalitySeperated(self):
        ...
        for modality in os.listdir(self.ModalityRootAfterInit):
            ...
            for dataset in os.listdir(
                ...
                dataset_class_map_path = os.path.join(
                    self.ModalityRootAfterInit,
                    modality,
                    dataset,
                    f"{dataset}_exist_class_map.json",
                )
                if os.path.exists(dataset_class_map_path):
                    modality_class_map[dataset] = json.load(
                        open(dataset_class_map_path, "r")
                    )
                    continue
                if os.path.isfile(
                    os.path.join(self.ModalityRootAfterInit, modality, dataset)
                ...
                results = p.imap_unordered(
                    self.GetUniqueLabelIndex_FromPNG, task_list, chunksize=20
                )

                for unique_index_per_mask in tqdm(
                    results, desc=f"{modality}_{dataset}", total=len(task_list)
                ):
                    for index in unique_index_per_mask:
                        if int(index) in dataset_exist_class_index:
                            continue
                        else:
                            dataset_exist_class_index.append(int(index))
                ...
                json.dump(
                    dataset_exist_class_map, open(dataset_class_map_path, "w"), indent=4
                )
            ...
            json.dump(
                ...
                open(
                    os.path.join(
                        self.ModalityRootAfterInit,
                        modality,
                        f"{modality}_exist_class_map.json",
                    ),
                    "w",
                ),
                indent=4,
            ...
            for dataset in tqdm(os.listdir(dataset_root), desc=modality):
                ...
                for Case in os.listdir(case_root):
                    ...
                    for direction in os.listdir(direction_root):
                        ...
                        case_slice_map[direction] = slice_name_to_slice_index(
                            slice_name_of_one_case
                        )
    ...
    def Create_Dataset_NumSamples_Map_for_Each_Dataset(self):
        ...
        for modality in tqdm(os.listdir(self.ModalityRootAfterInit), desc="Modality"):
            ...
            modality_num_slice_map["all"][modality] = sum(
                [count for dataset, count in dataset_num_slice_map.items()]
            )
    ...
    def Create_AtomClassLabelMap(self):
        ...
        for modality in os.listdir(self.ModalityRootAfterInit):
            ...
            for dataset in os.listdir(
                ...
                atom_class_map_path = os.path.join(
                    self.ModalityRootAfterInit,
                    modality,
                    dataset,
                    f"{dataset}_atom_class_map.json",
                )
                if os.path.exists(atom_class_map_path) or os.path.isfile(
                    os.path.join(self.ModalityRootAfterInit, modality, dataset)
                ):
                    continue

                dataset_class_map = json.load(
                    open(
                        os.path.join(
                            self.ModalityRootAfterInit,
                            modality,
                ...
                json.dump(atom_classes_dict, open(atom_class_map_path, "w"), indent=4)
    ...
    def Create_UnionClass_AtomClass_Rectiry_Map(self):
        ...
        def analyze_major_class(
            atom_class_list: Iterable, union_label_name: str
        ) -> str:
            assert union_label_name.startswith(
                "union_"
            ), f"Only able to analyze union class name, but got {union_label_name}"
            ...
            major_sub_class_name = max(
                sub_class_count, key=lambda k: sub_class_count[k]
            )
            return major_sub_class_name

        def rectify_one_dataset_class_map(
            union_class_map: Dict, atom_class_map: Dict
        ) -> Dict:
            # Create class rectify map for non-union labels
            class_names = union_class_map.keys()
            atom_class_names = [
                name for name in class_names if not name.startswith("union_")
            ]
            atom_class_map = {
            ...
            for class_name in class_names:
                if class_name.startswith("union_"):
                    major_class_name = analyze_major_class(
                        atom_class_map.keys(), class_name
                    )
                    class_rectify_map[union_class_map[class_name]] = atom_class_map[
                        major_class_name
                    ]
                else:
        ...
        for modality in os.listdir(self.ModalityRootAfterInit):
            ...
            for dataset in tqdm(datasets, desc=modality, total=len(datasets)):
                ...
                if not os.path.isdir(dataset_path):
                    continue
                rectify_map_path = os.path.join(
                    dataset_path, f"{dataset}_union_atom_class_rectify_map.json"
                )

                union_class_map = json.load(
                    open(
                        os.path.join(dataset_path, f"{dataset}_exist_class_map.json"),
                        "r",
                    )
                )
                atom_classes_map = json.load(
                    open(
                        os.path.join(dataset_path, f"{dataset}_atom_class_map.json"),
                        "r",
                    )
                )
                class_rectify_map = rectify_one_dataset_class_map(
                    union_class_map, atom_classes_map
                )
                json.dump(class_rectify_map, open(rectify_map_path, "w"), indent=4)

    def Create_NumCase_AvgSlice_Map(self):
        Slices_Dict = json.loads(
            open(
                os.path.join(self.ModalityRootAfterInit, f"NumSlice_map.json"), "r"
            ).read()
        )
        ...
        for modality in tqdm(os.listdir(self.ModalityRootAfterInit), desc="Modality"):
            ...
            for dataset in tqdm(os.listdir(dataset_root), desc=modality):
                ...
                dataset_num_cases_map["Avg_Slices_Per_Case"][dataset] = round(
                    num_slices_dataset / count, 2
                )

            modality_num_slice_map["_Num_Cases"][modality] = sum(
                dataset_num_cases_map["Num_Cases"].values()
            )
            modality_num_slice_map["_Avg_Slices_Per_Case"][modality] = np.mean(
                list(dataset_num_cases_map["Avg_Slices_Per_Case"].values())
            )
            modality_num_slice_map[modality] = dataset_num_cases_map
            json.dump(
                dataset_num_cases_map,
                open(
                    os.path.join(dataset_root, f"{modality}_NumCase_AvgSlice.json"), "w"
                ),
                indent=4,
            )
        json.dump(
            modality_num_slice_map,
            open(
                os.path.join(self.ModalityRootAfterInit, f"NumCase_AvgSlice.json"), "w"
            ),
            indent=4,
        )
            ...
            for dataset in datasets:
                ...
                for Case in cases:
                    ...
                    for direction in os.listdir(direction_root):
                        ...
                        for slice in slices:
                            if slice.endswith(".png") and slice.startswith("image_"):
                                distribution_one_slice = pool.submit(
                                    self.calculate_distributions_for_one_slice,
                ...
                json.dump(dataset_wise_distributions, open(save_path, "w"), indent=4)
        ...
        for modality in modalities:
            ...
            for dataset in datasets:
                ...
                save_path = os.path.join(
                    case_root, f"{dataset}_Label_Distributions.json"
                )
                ...
                sorted_dict = dict(
                    sorted(dataset_label_distributions.items(), key=lambda x: x[0])
                )
    ...
    def init_image(self):
        ...
        try:
            for index in tqdm(
                ...
                shutil.move(
                    os.path.join(self.image_path, image_list[index]),
                    os.path.join(
                        save_root, f"image_{file_meta_now['slice_index']}.png"
                    ),
                )
    ...
    def init_mask(self):
        def is_same(file_meta_1, file_meta_2, level):
            if file_meta_1 == {} or file_meta_2 == {}:
                return False
            # 前三项相同即为同一个序列
            for i in self.MASK_NAME_DEFINITION[:level]:
                if file_meta_1[i] != file_meta_2[i]:
                    return False
            return True
        ...
        try:
            for index in tqdm(
                ...
                if len(cache_save_result) // 1000 > 0:
                    for result in cache_save_result:
                        result.result()
                    cache_save_result: List[Future[bool]] = []

                # 如果发现了新的slice, 生成一个slice的mask
                if (
                    not is_same(file_meta_last, file_meta_now, -2)
                    and len(cache_slice_segmentation) > 0
                ) or index == num_mask_files - 1:
                    mask_of_slice = self.instance_to_segmentation(
                        cache_slice_segmentation
                    )
                    cache_slice_segmentation = []
                    ...
                    # 以png格式存储
                    save_result = pool_process_2.submit(
                        cv2.imwrite,
                        sava_path,
                        mask_of_slice,
                ...
                slice_segmentation_result = pool_process_1.submit(
                    self.png_to_ndarray, os.path.join(self.mask_path, mask_list[index])
                )
                cache_slice_segmentation.append(
                    (file_meta_now["class_instance"], slice_segmentation_result)
                )
