...
class SA_Med2D_Dataset(SA_Med2D, BaseSegDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.selected_dataset_root = os.path.join(
            self.structured_npz_root, modality, dataset_source
        )
        assert os.path.exists(
            self.selected_dataset_root
    ...
    def _structured_npz_root(root_path_mode: str):
        if root_path_mode == "from_linux":
            ...
            raise NotImplementedError(
                f"Unknown root_path_mode: {root_path_mode}, should be either 'from_linux' or 'from_mnt'."
            )
    ...
    def _init_index_map(selected_dataset_root, dataset_source):
        ...
        case_slice_map: Dict[str, Dict[str, List[str]]] = orjson.loads(
            open(
                os.path.join(
                    selected_dataset_root, f"{dataset_source}_CaseSlice_map.json"
                ),
                "r",
            ).read()
        )
        ...
        dataset_distributions: Dict[str, float] = orjson.loads(
            open(
                os.path.join(
                    selected_dataset_root, f"{dataset_source}_Distributions.json"
                ),
                "r",
            ).read()
        )
    ...
    def set_proxy(cls, modality, dataset_source, union_atom_rectify):
        if os.path.exists(
            os.path.join(cls._structured_npz_root("from_mnt"), modality, dataset_source)
        ):
            selected_dataset_root = os.path.join(
                cls._structured_npz_root("from_mnt"), modality, dataset_source
            )
        elif os.path.exists(
            os.path.join(
                cls._structured_npz_root("from_linux"), modality, dataset_source
            )
        ):
            selected_dataset_root = os.path.join(
                cls._structured_npz_root("from_linux"), modality, dataset_source
            ...
            raise FileNotFoundError(
                f"DataSource Not Available. Modality:{modality} | dataset name: {dataset_source} | union_atom_rectify:{union_atom_rectify}"
            )
        ...
        if union_atom_rectify:  # 可选纠正SA-Med2D数据集中的大量异常Union Label
            label_map = orjson.loads(
                open(
                    os.path.join(
                        selected_dataset_root, f"{dataset_source}_atom_class_map.json"
                    ),
                    "r",
                ).read()
            )
            ...
            label_map = orjson.loads(
                open(
                    os.path.join(
                        selected_dataset_root, f"{dataset_source}_exist_class_map.json"
                    ),
                    "r",
                ).read()
            )
    ...
    def slice_series_fetcher(self):
        ...
        if (
            (self.split != "test")
            and (self.activate_case_ratio is not None)
            and (self.activate_case_ratio < 1.0)
        ):
            assert (
                0 < self.activate_case_ratio <= 1
            ), f"activate_case_ratio must be in (0, 1]."
            target_num_used_cases = int(len(used_case_names) * self.activate_case_ratio)
            if target_num_used_cases == 0:
                target_num_used_cases = 1
            used_case_names = self.sampling_from_set(
                used_case_names, target_num_used_cases
            )

        for Case in tqdm.tqdm(
            used_case_names,
            desc=f"SA-Med2D | {self.modality} | {self.dataset_source} | {self.split}",
        ):
            ...
            for direction in os.listdir(direction_root):
                ...
                if not os.path.exists(slice_root):
                    raise FileNotFoundError(f"{slice_root} does not exist.")
...
class SA_Med2D_Dataset_MultiSliceSample(SA_Med2D_Dataset):
        ...
        **kwargs,
        ...
        if stride is not None:
            ...
            if isinstance(max_sample_per_case, float):
                ...
                raise ValueError(
                    f"If fixed per-case sampling is deployed,"
                    f"stride must be positive and int, "
                    f"but got {max_sample_per_case}"
                )
        assert slice_gap >= 1 and isinstance(
            slice_gap, int
        ), f"slice_gap must be positive and int, but got {slice_gap}"
        assert (
            num_images_per_sample % 2 == 1 and num_labels_per_sample % 2 == 1
    ...
    def _construct_sample_dict(
        self, slice_root, avail_idx, center_idx, image_idxs, label_idxs
    ):
        data_info = {
    ...
    def load_data_list(self) -> List:
        ...
        # 采样时，两个集合中心对齐，两侧对称
        max_gap_to_center = (
            max(self.num_images_per_sample, self.num_labels_per_sample) // 2
        ) * self.slice_gap
        mmseg_sample_list = []
        ...
        for slice_root, Case, direction, avail_idx in self.slice_series_fetcher():
            ...
            if self.max_sample_per_case is not None:
                avail_center_idx = range(
                    max_gap_to_center, num_samples - max_gap_to_center
                )
                ...
                if self.split != "test":
                    if isinstance(self.max_sample_per_case, int):
                        num_samples = min(
                            self.max_sample_per_case, len(avail_center_idx)
                        )
                    elif isinstance(self.max_sample_per_case, float):
                        num_samples = int(
                            len(avail_center_idx) * self.max_sample_per_case
                        )
                    else:
                        raise TypeError(
                            f"Unexpected type of max_sample_per_case: {self.max_sample_per_case}"
                        )
                    try:
            ...
            for center_idx in avail_center_idx:
                negative_image_idx = (
                    center_idx - self.num_images_per_sample // 2 * self.slice_gap
                )
                positive_image_idx = (
                    center_idx + self.num_images_per_sample // 2 * self.slice_gap
                )
                image_idx_of_this_sample = avail_idx[
                    negative_image_idx : positive_image_idx + 1 : self.slice_gap
                ]
                negative_label_idx = (
                    center_idx - self.num_labels_per_sample // 2 * self.slice_gap
                )
                positive_label_idx = (
                    center_idx + self.num_labels_per_sample // 2 * self.slice_gap
                )
                label_idx_of_this_sample = avail_idx[
                    negative_label_idx : positive_label_idx + 1 : self.slice_gap
                ]
                ...
                sample = self._construct_sample_dict(
                    ...
                    avail_idx,
                    center_idx,
                    image_idx_of_this_sample,
                    label_idx_of_this_sample,
                )
        ...
        print_log(
            msg=f"SA-Med2D | {self.modality} | {self.dataset_source} | {self.split} | Num Samples: {len(mmseg_sample_list)}",
            logger="current",
            level=logging.INFO,
        )
        class_map: Dict = self._metainfo["classes"]
        print_log(
            msg=f"SA-Med2D | {self.modality} | {self.dataset_source} | {len(class_map)} atom classes are {class_map}",
            logger="current",
            level=logging.INFO,
        )
...
class DatasetBackend_GlobalProxy(ManagerMixin):
    def __init__(
        self, name: str, union_atom_map=None, atom_classes=None, union_classes=None
    ) -> None:
        super().__init__(name)
