...
class SegVisualizationHook_SA_Med2D(SegVisualizationHook):
    ...
    def _after_iter(self, runner, batch_idx, data_batch, outputs, mode) -> None:
        ...
        if self.every_n_inner_iters(batch_idx, self.interval):
            for output in outputs:
                img_path = os.path.join(
                    ...
                    f"image_{output.img_path[1]}.png",
                )  # type: ignore
                img = Load_SA_Med2D.load_png(img_path)
                img = cv2.resize(
                    img, self.reshape_size, interpolation=cv2.INTER_NEAREST
                )
...
class IoU_SupportsMultipleIgnoreIndex(IoUMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if self.valid_label_on_summarizer is not None:
            ...
            total_area_intersect = torch.gather(
                total_area_intersect, 0, NonZero_Class_Index
            )
            total_area_union = torch.gather(total_area_union, 0, NonZero_Class_Index)
            total_area_pred_label = torch.gather(
                total_area_pred_label, 0, NonZero_Class_Index
            )
...
class Load_SA_Med2D_SingleSlice(Load_SA_Med2D):
    def transform(self, results: Dict) -> Dict:
        if "image" in self.load_type:
            ...
            image = self.load_png(
                os.path.join(
                    results["img_path"][0], f"image_{results['img_path'][1]}.png"
                )
            )
        ...
        if "label" in self.load_type:
            ...
            label = self.load_png(
                os.path.join(
                    results["seg_map_path"][0], f"mask_{results['seg_map_path'][1]}.png"
                )
            )
...
class Load_SA_Med2D_MultiSlices(Load_SA_Med2D):
    def __init__(self, multi_img_load: bool, lazy_load: bool = False, *args, **kwargs):
        ...
        super().__init__(
            *args, **kwargs
        )  # 多样本采样时，默认不支持在Load阶段进行归一化

    def transform(self, results: Dict) -> Dict:
        assert isinstance(results["multi_img_path"], List) and isinstance(
            results["multi_seg_map_path"], List
        ...
        if "image" in self.load_type:
            ...
            if self.multi_img_load:
                for img_idx in results["multi_img_path"]:
                    img_cache.append(
                        self.load_png(
                            os.path.join(results["img_path"][0], f"image_{img_idx}.png")
                        )
                    )
            else:  # 仅加载Axial Image
                img_cache.append(
            ...
            img_ndarray = (
                np.array(img_cache).transpose(1, 2, 0, 3).reshape(H, W, S * C)
            )  # (H,W,S*C)
        ...
        if "label" in self.load_type:
            ...
            if self.multi_img_load:
                for label_idx in results["multi_seg_map_path"]:
                    label_cache.append(
                        self.load_png(
                            os.path.join(
                                results["seg_map_path"][0], f"mask_{label_idx}.png"
                            )
                        )
                    )
            else:  # 仅加载Axial Label
            ...
            assert (
                label_cache.ndim == 3
            ), f"Label should be 3-dim, but got {label_cache.shape}"
            results["gt_seg_map"] = label_cache
            results["reduce_zero_label"] = (
                False  # 不可以随意更改此参数，会引起mmseg中多个行为改变
            )
...
class Normalize(BaseTransform):
    ...
    def transform(self, results: Dict):
        ...
        if self.mode == "dataset":
            ...
            image = (
                image / std
            )  # ignore mean calibration to prevent the background area offset to none zero value

        elif self.mode == "instance":
            image = cv2.normalize(
                image, None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_32FC3
            )
...
class Normalize_MultiSlice(Normalize):
    def transform(self, results: Dict):
        ...
        if self.mode == "dataset":
            ...
            for s in range(S):
                image[:, :, s] = cv2.normalize(
                    image[:, :, s], None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_32FC3
                )
...
class ForceResize(BaseTransform):
    def __init__(
        self, image_size: Tuple[int, int] = None, label_size: Tuple[int, int] = None
    ):  # type:ignore
    ...
    def transform(self, results: Dict):
        if self.image_size:
            ...
            results["img"] = cv2.resize(
                results["img"], dsize=self.image_size, interpolation=cv2.INTER_NEAREST
            )
    ...
    def transform(self, results):
        try:
            ...
            if seg_map.max() > self.max_cls_idx:
                raise ValueError(
                    f"Class index {seg_map.max()} is larger than valid maximum {self.max_cls_idx}"
                )
