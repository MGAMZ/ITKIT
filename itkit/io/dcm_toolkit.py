...
def read_dcm_as_sitk(
    data_directory: str, need_dcms: bool = True
    ...
    series_IDs = sitk.ImageSeriesReader.GetGDCMSeriesIDs(
        data_directory, useSeriesDetails=True
    )
    if not series_IDs:
        print(
            f"ERROR: given directory `{data_directory}` does not contain a DICOM series."
        )
        return None, None

    series_file_names = sitk.ImageSeriesReader.GetGDCMSeriesFileNames(
        data_directory, series_IDs[0], useSeriesDetails=True
    )
