...
def evaluation_dice(gt_data: np.ndarray, pred_data: np.ndarray):
    ...
    dice = (
        1
        - dice_loss(gt_class[None], pred_class[None], weight=None, ignore_index=None)
        .cpu()
        .numpy()
    )
    return dice
...
def evaluation_hausdorff_distance_3D(
    gt, pred, percentile: int = 95, interpolation_ratio: float | None = None
):
    ...
    pred = torch.from_numpy(pred).to(
        dtype=torch.uint8, device=f"cuda:{selected_device_id}"
    )
...
class SplitZ_Loss(torch.nn.Module):
    ...
    def forward(self, pred: Tensor, target: Tensor, *args, **kwargs):
        ...
        if pred_spatial_shape != target_spatial_shape:
            raise ValueError(
                f"Spatial dimensions of pred {pred.shape} and target {target.shape} must match."
            )
        if self.to_onehot_y:
        ...
        if self.split_Z:
            z_slice_losses = [
                self.monai_loss(p, t)
                for p, t in zip(
                    pred.permute(2, 0, 1, 3, 4), target.permute(2, 0, 1, 3, 4)
                )
            ]
...
class CrossEntropyLoss_3D(torch.nn.CrossEntropyLoss):
    ...
    def forward_one_patch(
        self, pred: Tensor, target: Tensor, weight: float | None = None, *args, **kwargs
    ):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(target.shape) == len(pred.shape):
            ...
            # target是类别索引
            assert (
                pred_spatial_shape == target_spatial_shape
