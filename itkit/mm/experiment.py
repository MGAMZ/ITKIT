...
class experiment:
    ...
    def _main_process(self):
        if self.IsTested(self.cfg):
            ...
            print_log(
                f"{Fore.BLUE}Test start: {self.work_dir}{Style.RESET_ALL}",
                "current",
                logging.INFO,
            )
            ...
            print_log(
                f"{Fore.GREEN}Test complete: {self.work_dir}{Style.RESET_ALL}",
                "current",
                logging.INFO,
            )
            ...
            print_log(
                f"{Fore.BLUE}Train start: {self.work_dir}{Style.RESET_ALL}",
                "current",
                logging.INFO,
            )
            runner.train()
    ...
    def _direct_to_test(self):
        ...
        if self.test_use_last_ckpt:
            ...
            assert (
                len(ckpt_path) == 1
            ), f"Tring to find best model at {ckpt_path}, but cannot determine which is the best one."
            ckpt_path = ckpt_path[0]
        print_log(f"Loading model checkpoint: {self.work_dir}", "current", logging.INFO)
        runner.load_checkpoint(ckpt_path)
        print_log(
            f"Model checkpoint loaded, doing test now: {self.work_dir}",
            "current",
            logging.INFO,
        )
