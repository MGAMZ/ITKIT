...
class IoUMetric_PerClass(IoUMetric):
    def __init__(
        self, iou_metrics: list[str] = ["mIoU", "mDice", "mFscore"], *args, **kwargs
    ):
...
class MonaiSegMetrics(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: list) -> None:
        ...
        for data_sample in data_samples:
            ...
            # recall = TPR = TP / (TP + FN)
            recall_score = compute_confusion_matrix_metric(
                "recall", confusion_matrix
            )  # [C]
            # precision = PPV = TP / (TP + FP)
            precision_score = compute_confusion_matrix_metric(
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        per_classes_formatted_dict["Dice"] = [
            format(v.item() * 100, ".2f") for v in dice_scores
        ]
        per_classes_formatted_dict["IoU"] = [
            format(v.item() * 100, ".2f") for v in iou_scores
        ]
        per_classes_formatted_dict["Recall"] = [
            format(v.item() * 100, ".2f") for v in recall_scores
        ]
        per_classes_formatted_dict["Precision"] = [
            format(v.item() * 100, ".2f") for v in precision_scores
        ]
    ...
    def _to_onehot(self, label_map: torch.Tensor, num_classes: int) -> torch.Tensor:
        """
