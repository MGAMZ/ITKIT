...
class Inferencer:
    def __init__(
        self, cfg_path, ckpt_path, fp16: bool = False, allow_tqdm: bool = True
    ):
        self.fp16 = fp16
...
class SegInferencer(Inferencer):
    def Inference_FromITK(self, itk_image: sitk.Image) -> tuple[sitk.Image, sitk.Image]:
        ...
        pred = (
            pred.argmax(dim=0).to(dtype=torch.uint8, device="cpu").numpy()
        )  # [Z, Y, X]
    ...
    def Inference_FromDcm(self, dcm_slide_folder: str, spacing=None):
        image, _, _, _ = LoadDcmAsSitkImage(
            "engineering", dcm_slide_folder, spacing=spacing
        )
...
class Inferencer_2D(SegInferencer):
    @torch.inference_mode()
    def Inference_FromNDArray(self, image_array: np.ndarray) -> Tensor:
        assert image_array.ndim == 3, "Input image must be (Z,Y,X), got: {}.".format(
            image_array.shape
        )
        ...
        for array, sample in tqdm(
            zip(inputs, data_samples),
            desc="Inferencing",
            total=len(inputs),
            dynamic_ncols=True,
            leave=False,
...
class Inferencer_3D(SegInferencer):
    def __init__(
        self, spacings=[None, None, None], sizes=[None, None, None], *args, **kwargs
    ):
        super().__init__(*args, **kwargs)
        assert (
            len(spacings) == 3
        ), "Spacings must be a list of 3 elements, got: {}.".format(spacings)
        assert len(sizes) == 3, "Sizes must be a list of 3 elements, got: {}.".format(
            sizes
        )
        assert not any(
            [
                spacing is not None and size is not None
                for spacing, size in zip(spacings, sizes)
            ]
        ), "Can not specify spacing and size for one dimension at the same time, got spacings: {}, sizes: {}.".format(
            spacings, sizes
        )
    ...
    def Inference_FromITK(self, itk_image: sitk.Image) -> tuple[sitk.Image, sitk.Image]:
        if any(self.spacings):
            ...
            target_spacing = [
                target or ori for ori, target in zip(ori_spacing, self.spacings)
            ]
...
class Inferencer_3D_ONNX(SegInferencer):
    ...
    def Inference_FromNDArray(self, image_array: np.ndarray) -> np.ndarray:
        ...
        image_array = SetWindow(image_array, self.ww, self.wl).astype(
            self.input_dtype
        )  # [1, C, Z, Y, X]
    ...
    def slide_inference(self, inputs: np.ndarray) -> np.ndarray:
        ...
        assert (
            self.inference_PatchSize is not None
            and self.inference_PatchStride is not None
        ...
        preds = np.zeros(
            (batch_size, out_channels, z_img, y_img, x_img), dtype=np.float16
        )
        ...
        assert np.all(
            count_mat > 0
        ), "There exists some voxels not covered by any patch, check slide inference logic or report this issue plz."
