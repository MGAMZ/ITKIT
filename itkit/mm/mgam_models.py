...
class ArgmaxProcessor:
    ...
    def argmax(self, logits: Tensor, dim: int = 1, keepdim: bool = True) -> Tensor:
        ...
        for start in range(0, L, chunk_size):
            ...
            preds_chunk = torch.argmax(t_chunk, dim=dim, keepdim=keepdim).to(
                torch.uint8
            )
...
class mgam_Seg_Lite(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.criterion = (
            [MODELS.build(c) for c in criterion]
            if isinstance(criterion, list)
            else [MODELS.build(criterion)]
        )
        ...
        if inference_config is None:
            ...
            raise TypeError(
                f"inference_config must be InferenceConfig, dict or None, but got {type(inference_config)}"
            )
    ...
    @abstractmethod
    def predict(
        self, inputs: Tensor, data_samples: Sequence[BaseDataElement] | None = None
...
class mgam_Seg2D_Lite(mgam_Seg_Lite):
    def loss(self, inputs: Tensor, data_samples: Sequence[BaseDataElement]) -> dict:
        ...
        return {
            "loss_" + cri.__class__.__name__: cri(seg_logits, gt_segs)
    ...
    def predict(
        ...
        if out_channels == 1 and self.binary_segment_threshold is None:
            raise ValueError(
                f"Binary model (out_channels={out_channels}) must set binary_segment_threshold; current value is None"
            )
        ...
        for i in range(batch_size):
            ...
            if out_channels > 1:  # 多分类情况
                ...
                i_seg_pred = (i_seg_logits_sigmoid > self.binary_segment_threshold).to(
                    i_seg_logits
                )
    ...
    def inference(
        ...
        # 检查是否需要滑动窗口推理
        if (
            self.inference_config.patch_size is not None
            and self.inference_config.patch_stride is not None
        ):
    ...
    def slide_inference(
        ...
        if not (
            isinstance(self.inference_config.patch_size, (tuple, list))
            and len(self.inference_config.patch_size) == 2
        ):
            raise AssertionError(
                f"For 2D inference, patch_size must be a tuple/list of length 2, got {self.inference_config.patch_size}"
            )
        if not (
            isinstance(self.inference_config.patch_stride, (tuple, list))
            and len(self.inference_config.patch_stride) == 2
        ):
            raise AssertionError(
                f"For 2D inference, patch_stride must be a tuple/list of length 2, got {self.inference_config.patch_stride}"
            )
        ...
        if need_padding:
            ...
            padded_inputs = torch.nn.functional.pad(
                inputs, pad, mode="replicate", value=0
            )
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                crop_seg_logit = self._forward(
                    crop_img.to(self.inference_config.forward_device)
                )
        ...
        assert (
            torch.min(count_mat).item() > 0
        ), "There are areas not covered by sliding windows"
        ...
        if need_padding:
            assert (
                pad is not None
            ), "Missing padding info, cannot crop back to original size"
...
class mgam_Seg3D_Lite(mgam_Seg_Lite):
    ...
    def predict(
        ...
        def _predict(force_cpu: bool = False):
            ...
            for i in range(batch_size):
                ...
                if out_channels > 1:  # Multi-class segmentation
                    ...
                    i_seg_logits_sigmoid = i_seg_logits.sigmoid()
                    i_seg_pred = (
                        i_seg_logits_sigmoid > self.binary_segment_threshold
                    ).to(i_seg_logits)
        ...
        try:
            ...
            print_log(
                "OOM during slide inference, trying cpu accumulate.",
                "current",
                logging.WARNING,
            )
    ...
    def slide_inference(
        ...
        data_samples: Sequence[BaseDataElement] | None = None,
        force_cpu: bool = False,
        forward_batch_windows: int | None = None,
    ) -> Tensor:
        ...
        batch_windows = (
            forward_batch_windows or self.inference_config.forward_batch_windows
        )
        batch_size, _, z_img, y_img, x_img = inputs.size()
        assert (
            batch_size == 1
        ), "Currently only batch_size=1 is supported for 3D sliding-window inference"
        ...
        if need_padding:
            ...
            z_padded, y_padded, x_padded = (
                padded_inputs.shape[2],
                padded_inputs.shape[3],
                padded_inputs.shape[4],
            )
        else:
            padded_inputs = inputs
        ...
        accumulate_device = (
            torch.device("cpu")
            if force_cpu
            else torch.device(self.inference_config.accumulate_device)
        )
        ...
        def _device_to_host_pinned_tensor(
            device_tensor: Tensor, non_blocking: bool = False
        ) -> Tensor:
            """Inplace ops on pinned tensor for efficient transfer."""
            nonlocal patch_cache, preds
            device_tensor = device_tensor.to(
                preds.dtype
            )  # NOTE Inconsistent dtype can SEVERLY impact tranfer speed.
        ...
        for i in tqdm(
            range(0, len(window_slices), batch_windows),
            ...
            batch_patches = torch.cat(batch_patches, dim=0).to(
                self.inference_config.forward_device
            )  # [B, C, z_crop, y_crop, x_crop]
            ...
            if (
                ...
                and torch.device(self.inference_config.forward_device).type == "cuda"
            ):
