...
class LocalVisBackend(_LocalVisBackend):
    def add_image(self, name: str, image: np.ndarray, step: int = 0, **kwargs) -> None:
        ...
        save_file_name = f"{name}_{step}.png".replace(
            "/", "__"
        )  # support working with tensorboard tag rule.
    ...
    def after_test_iter(
        ...
        for i in range(len(outputs)):
            if batch_idx % self.test_vis_interval == 0 and self.enabled:
                ...
                self._visualizer.add_datasample(
                    window_name, data_batch["inputs"][i], data_sample=outputs[i], step=0
                )
...
class SegViser(BaseViser):
        ...
        def to_ndarray(key):
            ...
            if isinstance(data, torch.Tensor):
                ...
                raise NotImplementedError(
                    f"Unsupported data type when visualizing: {type(data)}"
                )
    ...
    def _draw_fig(
        ...
        if gt_seg_map is not None:
            ...
            axes[1].imshow(
                gt_seg_map,
                cmap=self.seg_map_cmap,
                interpolation="nearest",
                alpha=self.seg_map_alpha,
            )
    ...
    def add_datasample(
        ...
        image, gt_seg_map, pred_seg_map, pred_seg_logits = self._parse_datasample(
            image, data_sample
        )
        if gt_seg_map is None:
            print_log(
                f"When visualizing `{name}` with img_path `{img_path}`, "
        ...
        # draw fig and save
        image_array = self._draw_fig(
            img_path, image, gt_seg_map, pred_seg_map, pred_seg_logits
        )
