        ...
        for s in range(1, n_stages_encoder):
            ...
            stages.append(
                VSSLayer(
                    ...
                    d_state=(
                        math.ceil(2 * input_features_skip / 6)
                        if d_state is None
                        else d_state
                    ),
                    norm_layer=nn.LayerNorm,
            ...
            seg_layers.append(
                nn.Conv2d(input_features_skip, num_classes, 1, 1, 0, bias=True)
            )
...
def load_pretrained_ckpt(
    ...
    for k, v in ckpt["model"].items():
        ...
        if (
            "patch_embed" in k
            and ckpt["model"]["patch_embed.proj.weight"].shape[1] != num_input_channels
        ):
