...
class ACBlock(nn.Module):
    ...
    def switch_to_deploy(self):
        ...
        self.fused_conv = nn.Conv2d(
            ...
            kernel_size=self.square_conv.kernel_size,  # pyright: ignore[reportArgumentType]
            stride=self.square_conv.stride,  # pyright: ignore[reportArgumentType]
            padding=self.square_conv.padding,  # pyright: ignore[reportArgumentType]
            dilation=self.square_conv.dilation,  # pyright: ignore[reportArgumentType]
    ...
    for k, p in test_kernel_padding:
        ...
        for module in acb.modules():
            if isinstance(module, nn.BatchNorm2d):
                nn.init.uniform_(module.running_mean, 0, 0.1)  # pyright: ignore[reportArgumentType]
                nn.init.uniform_(module.running_var, 0, 0.2)  # pyright: ignore[reportArgumentType]
        ...
        deployout = acb(x)
        print(
            "difference between the outputs of the training-time and converted ACB is"
        )
