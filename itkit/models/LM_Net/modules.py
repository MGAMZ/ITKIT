...
class OverlapPatchEmbed(nn.Module):
    def __init__(
        self, patchsize, img_size, in_channels, embed_dim, stride, model="no nat"
    ):  # 8,32,128
        super().__init__()
        self.model = model
...
class NoskipViTEncoder(nn.Module):
    def __init__(self, patchsize, img_size, in_channels, stride, kernel_size, head):
        ...
        self.patchembedding_l = OverlapPatchEmbed(
            patchsize, img_size, in_channels, in_channels, stride
        )
        self.patchembedding_s = OverlapPatchEmbed(
            patchsize, img_size, in_channels, in_channels, stride
        )
        self.norm_l1 = nn.LayerNorm(in_channels)
        self.norm_s1 = nn.LayerNorm(in_channels)
        self.cross = NEWNeighborhoodAttention(
            in_channels, kernel_size, head, attn_drop=0.1, proj_drop=0.1
        )
    ...
    def forward(self, xq, xkv):
        ...
        xq_embedding, xkv_embedding = self.patchembedding_s(xq), self.patchembedding_l(
            xkv
        )
...
class M2Skip(nn.Module):
    def __init__(self, in_channels=[16, 32], model_type="bottom"):  # 大,小
        ...
        if self.model_type == "bottom":
            ...
            self.fuse_conv = nn.Sequential(
                nn.Conv2d(2 * in_channels[1], in_channels[1], 3, 1, 1),
                nn.BatchNorm2d(in_channels[1]),
                nn.GELU(),
                # SPBlock(in_channels[1],in_channels[1])
            )
            ...
            self.fuse_conv = nn.Sequential(
                nn.Conv2d(2 * in_channels[0], in_channels[0], 3, 1, 1),
                nn.BatchNorm2d(in_channels[0]),
                nn.GELU(),
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=7, img_size=224, in_chans=3, out_channel=768):
        ...
        self.position_embeddings = nn.Parameter(
            torch.zeros(1, self.num_patches, out_channel)
        )
...
class PoolTransformer(nn.Module):
    def __init__(
        ...
        self.patchembedding = OverlapPatchEmbed(
            patchsize, img_size, in_channels, out_channel, stride
        )
        self.norm1 = nn.LayerNorm(out_channel)
        self.attention = PoolingAttention(
            out_channel, num_heads, pool_ratios=pool_ratios
        )
    ...
    def forward(self, x):
        ...
        att = (
            self.drop_path(
                self.attention(self.norm1(x_embedding), self.hw, self.hw, self.d_conv)
            )
            + x_embedding
        )
...
class NAT_Global_Transformer(nn.Module):
    def __init__(
        ...
        self.patchembedding1 = OverlapPatchEmbed(
            3, img_size, in_channels, out_channel, 1
        )
        self.patchembedding3 = OverlapPatchEmbed(
            3, img_size, in_channels, out_channel, 1
        )
        self.patchembedding2 = OverlapPatchEmbed(
            patchsize, img_size, in_channels, out_channel, stride
        )
        self.norm1 = nn.LayerNorm(out_channel)
...
class PyramidPool(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        B, C, H, W = x5.shape
        x = torch.cat(
            [nn.functional.adaptive_avg_pool2d(i, (H, W)) for i in [x1, x2, x3, x4]],
            dim=1,
        )
...
class NeighborhoodTransformer(nn.Module):
    def __init__(
        ...
        self.img_size = img_size
        self.patchembedding = OverlapPatchEmbed(
            patchsize, img_size, in_channels, out_channel, stride, "nat"
        )
        self.norm1 = nn.LayerNorm(out_channel)
        self.att1 = NeighborhoodAttention2D(dim=out_channel, num_heads=8, kernel_size=7)
        self.att2 = NeighborhoodAttention2D(
            dim=out_channel,
            num_heads=8,
            kernel_size=3,
        )
...
class ReparamConv(nn.Module):
    ...
    def __init__(
        ...
        self.pointwise_conv = nn.Sequential(
            nn.Conv2d(
                expand_channels, out_channels, kernel_size=1, stride=1, padding=0
            ),
    ...
    def get_equivalent_kernel_bias(self):
        ...
        large_k += nn.functional.pad(
            square_k, [(self.large_kernel_size - self.kernel_size) // 2] * 4
        )
...
class MobileBlock(nn.Module):
    ...
    def __init__(
        ...
        self.expand_conv = nn.Sequential(
            nn.Conv2d(
                in_channels, expand_channels, kernel_size=1, stride=1, bias=False
            ),
            nn.BatchNorm2d(expand_channels),
            nn.Hardswish(inplace=True),
        )
...
class SegHead(nn.Module):
    def __init__(self, in_channels=[16, 32, 64, 128]):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(4 * in_channels[3], in_channels[0], 1),
            nn.BatchNorm2d(in_channels[0]),
        )
        self.dropout = nn.Dropout(0.1)
...
class DeformConv(nn.Module):
    def __init__(
        self, input_dim, output_dim, stride=1, padding=[1, 1], dilation=[1, 1]
    ):
...
class SPBlock(nn.Module):  # Strip Pooling
    def __init__(self, inplanes, outplanes, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(
            inplanes, midplanes, kernel_size=(3, 1), padding=(1, 0), bias=False
        )

        self.bn1 = nn.BatchNorm2d(midplanes)
        self.conv2 = nn.Conv2d(
            inplanes, midplanes, kernel_size=(1, 3), padding=(0, 1), bias=False
        )
...
class My_ASPP(nn.Module):
    def __init__(self, in_dims, out_dims, rate=[1, 6, 12, 18]):
        ...
        self.global_avg_pool = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),  # 输出1*1的特征图
            nn.Conv2d(in_dims, out_dims, 1, stride=1),  # 1*1的卷积
            nn.BatchNorm2d(out_dims),
            nn.ReLU(inplace=True),
        )
        self.output = nn.Sequential(
            nn.Conv2d((len(rate) + 1) * out_dims, out_dims, 1),
            nn.BatchNorm2d(out_dims),
            nn.ReLU(inplace=True),
        )
        self._init_weights()
...
class Up(nn.Module):
    def __init__(self, input_decoder, output_dim):
        ...
        self.conv_decoder = nn.Sequential(
            nn.ConvTranspose2d(
                input_decoder, output_dim, kernel_size=2, stride=2, padding=0
            ),
            nn.BatchNorm2d(output_dim),
            nn.ReLU(),
        )
...
class Carafe_Up(nn.Module):
    def __init__(
        self, input_decoder, output_dim, compressed_channels=64, scale_factor=2
    ):
        super(Carafe_Up, self).__init__()
        self.carafe_up = nn.Sequential(
