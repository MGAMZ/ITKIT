...
if is_ninja_available():
    ...
    nattenav_cuda = load(
        "nattenav_cuda",
        [
            f"{this_dir}/src/nattenav_cuda.cpp",
            f"{this_dir}/src/nattenav_cuda_kernel.cu",
        ],
        verbose=False,
    )
    nattenqkrpb_cuda = load(
        "nattenqkrpb_cuda",
        [
            f"{this_dir}/src/nattenqkrpb_cuda.cpp",
            f"{this_dir}/src/nattenqkrpb_cuda_kernel.cu",
        ],
        verbose=False,
    )
else:
    try:
        ...
        raise RuntimeError(
            "Could not load NATTEN CUDA extension. "
...
class NATTENAVFunction(Function):
    ...
    def forward(ctx, attn, value):
        ...
        out = nattenav_cuda.forward(attn, value)
...
class NATTENQKRPBFunction(Function):
    ...
    def forward(ctx, query, key, rpb):
        ...
        key = key.contiguous()
        attn = nattenqkrpb_cuda.forward(query, key, rpb)
