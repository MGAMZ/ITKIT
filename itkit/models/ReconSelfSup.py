...
class PackReconInput(BaseTransform):
    def transform(self, results: dict):
        ...
        datasample = ReconDataSample(
            ...
            metainfo={
                "sample_file_path": results["img_path"],
            },
        )
...
class ReconHead(BaseModule):
    def __init__(
        ...
        self.conv_proj = eval(f"nn.Conv{dim}")(model_out_channels, recon_channels, 1)

    def loss(
        self, recon: Tensor, ori: Tensor, mask: Tensor | None = None
    ) -> dict[str, Tensor]:
        proj = self(recon)
...
class Reconstructor(AutoEncoderSelfSup):
    ...
    def _stack_datasamples(
        self, data_samples: list[ReconDataSample]
    ) -> tuple[Tensor, Tensor]:
    ...
    def loss(
        self, inputs: list[Tensor], data_samples: list[ReconDataSample]
    ) -> dict[str, Tensor]:
        ...
        for z_idx in range(z_grids):
            for y_idx in range(y_grids):
                for x_idx in range(x_grids):
                    ...
                    preds[:, :, z1:z2, y1:y2, x1:x2] += reconed.to(
    ...
    def predict(
        self, inputs: Tensor, data_samples: list[ReconDataSample]
    ...
    def forward(
        self, inputs: Tensor, data_samples: list[ReconDataSample], mode: str = "tensor"
    ):
        if mode == "loss":
            ...
            self.results.append(
                {
                    ...
                    "mape": torch.mean(torch.abs(pred - gt) / (gt + self.eps))
                    .cpu()
                    .numpy(),
                }
            )
