...
class SegFormer3D_Encoder_MM(BaseModule):
    ...
    def forward(self, x):
        ...
        for stage_idx in range(4):
            ...
            x = (
                x.reshape(B, *patched_volume_size, C)
                .permute(0, 4, 1, 2, 3)
                .contiguous()
            )
            out.append(x)
...
class SegFormer3D_Decoder_MM(BaseDecodeHead_3D):
    ...
    def forward(self, *args, **kwargs):
        ...
        if num_input_elements == 1:
            assert (
                len(args[0]) == 4
            ), f"Invalid number of inputs for SegFormer3D_Decoder_MM: {len(args[0])}"
            segformer_out = self.segformer.forward(*args[0])
        elif num_input_elements == 4:
            segformer_out = self.segformer.forward(*args)
        else:
            raise ValueError(
                f"Invalid number of inputs for SegFormer3D_Decoder_MM: {num_input_elements}"
            )
