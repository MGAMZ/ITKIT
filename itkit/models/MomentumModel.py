...
class MomentumAvgModel(torch.nn.Module):
    def __init__(
        ...
        self.is_deepspeed = hasattr(model, "module") and hasattr(
            model.module, "deepspeed"
        )
        ...
        if update_buffers:
            ...
            self.avg_parameters = {k: v for k, v in state_dict.items() if v.numel() > 0}
        else:
            params = dict(self.module.named_parameters())
            self.avg_parameters = {k: v for k, v in params.items() if v.numel() > 0}

        # Validate momentum parameter range
        assert (
            0.0 < momentum < 1.0
        ), f"momentum must be in range (0.0, 1.0) but got {momentum}"
        if momentum > 0.5:
            print_log(
    ...
    def update_parameters(self, model: torch.nn.Module) -> None:
        ...
        if self.steps == 0:
            ...
            for k, p_avg in self.avg_parameters.items():
                ...
                if (
                    p_avg.dtype.is_floating_point
                    and p_avg.shape == src_parameters[k].data.shape
                ):
                    device = p_avg.device
                    self.avg_func(
