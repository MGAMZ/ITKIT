...
CONV_MAPPING = {"3d": nn.Conv3d, "2d": nn.Conv2d, "1d": nn.Conv1d}
...
class RandomVolumeView(BaseTransform):
    ...
    def __init__(
        self, num_views: int, dim: Literal["1d", "2d", "3d"], size: tuple[int]
    ):
...
class ParseCoords(BaseTransform):
    ...
    def _get_AdjDst_indices(self, abs_gap: np.ndarray, coords: np.ndarray) -> tuple:
        ...
        for idx_pair, (i, j) in enumerate(pairs):
            ...
            indices[idx_pair] = [
                ...
                + tuple(
                    slice(
                        v_c[x] + offset[x] - sub_v_c[x], v_c[x] + offset[x] + sub_v_c[x]
                    )
    ...
    def _parse_gap(self, coords: np.ndarray):
        ...
        rel_gap = abs_gap / (
            rel_base[None, None, ...] + 1e-5
        )  # normalize the gap matrix
    ...
    def _parse_route(self, direction: np.ndarray) -> np.ndarray:
        ...
        def _calc_destination(
            routes: np.ndarray, coords: np.ndarray, gapBetweenCoords: np.ndarray
        ):
            """
    ...
    def transform(self, results: dict):
        ...
        results["sim_pair_indices"], results["sim_pair_centers"] = (
            self._get_AdjDst_indices(abs_gap, coords)
        )
        results["normed_sim_pair_indices"], results["normed_sim_pair_centers"] = (
            self._get_AdjDst_indices(normed_abs_gap, normed_coords)
        )
...
class RelativeSimilaritySelfSup(AutoEncoderSelfSup):
    ...
    def __init__(
        ...
        extra_keys: list[str] = [
            ...
            "normed_abs_gap",
            "normed_rel_gap",
            "normed_sim_pair_indices",
            "normed_sim_pair_centers",
        ],
        *args,
        **kwargs,
    ):
    ...
    def extract_nir(self, vv_main: Tensor, vv_aux: Tensor) -> Tensor:
        ...
        aux_model = self.momentum_encoder if self.momentum else self.whole_model_
        with torch.no_grad():
            nir_aux = [aux_model(view)[0] for view in vv_aux.transpose(0, 1)]
    ...
    def predict(
        ...
        sim_pred, sim_loss = self.sim_head.predict(
            nir, coord_info["sim_pair_indices"], coord_info["sim_pair_centers"]
        )
        # [N, view, view, 3]
...
class GlobalAvgPool(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.dim == "1d":
            ...
            return (
                nn.functional.adaptive_avg_pool3d(x, (1, 1, 1))
                .squeeze(-1)
                .squeeze(-1)
                .squeeze(-1)
            )
...
class BaseVolumeWisePredictor(nn.Module):
    ...
    def __init__(
        self, dim: Literal["1d", "2d", "3d"], in_channels: int, num_views: int = 3
    ):
        super().__init__()
...
class GapPredictor(BaseVolumeWisePredictor):
    ...
    def __init__(self, loss_weight: float = 1.0, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.cri = nn.MSELoss()
    ...
    def loss(self, nir: Tensor, gap: Tensor) -> dict[str, Tensor]:
        ...
        return {
            "loss/axis_gap": loss_axis_gap * self.loss_weight,
            "loss/norm_gap": loss_norm_gap * self.loss_weight,
        }

    @torch.inference_mode()
    def predict(
        self, nir: Tensor, gap: Tensor | None = None
...
class SimPairDiscriminator(BaseModule):
    ...
    def __init__(
        ...
        self.view_size = np.array(
            [view_size] * DIM_MAP[dim] if isinstance(view_size, int) else view_size
        )

        if len(self.sub_view_size) != DIM_MAP[dim]:
            raise ValueError(
                f"sub_volume_size length {len(self.sub_view_size)} does not match dim {dim}"
            )
    ...
    def loss(self, nir: Tensor, sub_area_indices: np.ndarray) -> dict[str, Tensor]:
        ...
        return {
            "loss/sim_adja": -adja_losses
            * self.loss_weight,  # 邻近对应该更相似（高similarity）
            "loss/sim_dist": dist_losses
            * self.loss_weight,  # 远离对应该更不相似（低similarity）
        }
    ...
    def _find_closest_farthest_pairs(self, f) -> tuple[Tensor, Tensor]:
        ...
        max_pairs = torch.stack(
            [self.pair_rows[max_idx], self.pair_cols[max_idx]], dim=-1
        )
        min_pairs = torch.stack(
            [self.pair_rows[min_idx], self.pair_cols[min_idx]], dim=-1
        )
    ...
    @torch.inference_mode()
    def predict(
        self, nir: Tensor, sub_view_indices: np.ndarray, sub_view_coords: np.ndarray
    ) -> tuple:
        """
        Args:
            nir (Tensor): [N, num_views, C, ...]
...
class VecAngConstraint(BaseVolumeWisePredictor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cycle_route_index = torch.from_numpy(
            self.generate_all_cycles(num_views)
        )  # [num_views, num_paths, path_steps, 2]
    ...
    def forward(self, nir: Tensor) -> Tensor:
        ...
        dire_vect_diff = dire_vect.unsqueeze(2) - dire_vect.unsqueeze(
            1
        )  # (N, num_views, num_views, C)
        ...
        for batch_idx in range(B):
            ...
            def backtrack(current: int, start: int, visited: set, path_vector: Tensor):
                ...
                for next_point in range(L):
                    if next_point not in visited:
                        ...
                        new_vector = (
                            path_vector
                            + direction_tensor[batch_idx, current, next_point]
                        )
                        backtrack(next_point, start, visited, new_vector)
                        visited.remove(next_point)
            ...
            for i in range(L):
                ...
                for j in range(L):
                    if i != j:
                        ...
                        if paths:
                            ...
                            end_paths.append(
                                torch.zeros((1, D), device=direction_tensor.device)
                            )
                    else:
                        # 对角线位置(起点=终点)填充零向量
                        end_paths.append(
                ...
                for paths in end_paths:
                    if paths.shape[0] < max_paths:
                        padding = torch.zeros(
                            (max_paths - paths.shape[0], D), device=paths.device
                        )
    ...
    def loss(self, nir: Tensor, gap: Tensor) -> dict[str, Tensor]:
        ...
        return {
            "loss/vect": loss_vect * self.loss_weight,
...
class RelSim_Metric(BaseMetric):
    def __init__(self, collect_device: str = "cpu", prefix: str = "Perf"):
    ...
    def _vis_sim(
        ...
        def draw_bbox(center, size, fill=False, fill_alpha=0.1, **kwargs):
            ...
            for dz in [-d // 2, d // 2]:
                for dy in [-h // 2, h // 2]:
                    for dx in [-w // 2, w // 2]:
                        corners.append([x_c + dx, y_c + dy, z_c + dz])
        ...
        def draw_surface(ax: Axes, entire_volume):
            ...
            ax.plot_surface(
                ...
                y,
                z,
                facecolors=plt.cm.gray(front_slice),
                alpha=0.3,
                edgecolor="none",
            )
            ...
            z = np.full_like(x, D)
            ax.plot_surface(
                x, y, z, facecolors=plt.cm.gray(top_slice), alpha=0.3, edgecolor="none"
            )
            ...
            ax.plot_surface(
                ...
                0,
                z,
                facecolors=plt.cm.gray(right_slice),
                alpha=0.3,
                edgecolor="none",
            )
    ...
    def add_datasample(self, data_sample: DataSample, step: int | None = None):
        ...
        Args:
            data_sample: datasample dict
                ...
                - sim_chunk_size (Tensor): [num_pairs, 3]
        """
        gap_vis_img = self._vis_gap(data_sample.gap_pred, data_sample.abs_gap)
        sim_vis_img = self._vis_sim(
            data_sample.volume,
        ...
        vec_vis_img = self._vis_vec(
            data_sample.vec_pred, data_sample.view_coords, data_sample.abs_gap
        )
