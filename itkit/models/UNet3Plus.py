...
class UNet3Plus(nn.Module):
    def __init__(
        ...
        self.e2 = nn.Sequential(
            MaxPool(2), ConvBlock(self.filters[0], self.filters[1], dim=dim)
        )
        self.e3 = nn.Sequential(
            MaxPool(2), ConvBlock(self.filters[1], self.filters[2], dim=dim)
        )
        self.e4 = nn.Sequential(
            MaxPool(2), ConvBlock(self.filters[2], self.filters[3], dim=dim)
        )
        self.e5 = nn.Sequential(
            MaxPool(2), ConvBlock(self.filters[3], self.filters[4], dim=dim)
        )
        ...
        self.d4_conv = ConvBlock(
            self.upsample_channels, self.upsample_channels, n=1, dim=dim
        )
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        if self.CGM:
            assert (
                self.cgm is not None
            ), "Classification Guided Module is enabled but CGM layer is not defined."
            cls = self.cgm(e5)
        ...
        if self.deep_supervision:
            assert (
                self.deep_sup is not None
            ), "Deep supervision is enabled but deep supervision layers are not defined."
            outputs.extend(
                [
...
if __name__ == "__main__":
    # 2D 示例
    unet_3P_2d = UNet3Plus(input_shape=[1, 320, 320], output_channels=4, dim=2)
    x_2d = torch.randn(3, 1, 320, 320)
    output_2d = unet_3P_2d(x_2d)
    print(f"2D 输出形状: {output_2d.shape}")

    # 3D 示例
    unet_3P_3d = UNet3Plus(input_shape=[1, 64, 64, 64], output_channels=4, dim=3)
