...
def ResampleFolder(
    ...
    for subfolder in subfolders:
        ...
        for file in files:
            tasks.append(
                (source_subfolder, target_subfolder, file, spacing, size, subfolder)
            )
    ...
    if use_mp:
        with Pool() as pool:
            list(
                tqdm(
                    pool.imap(process_file, tasks), total=len(tasks), desc="Processing"
                )
            )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "source_folder",
        type=str,
        help="Path to the source folder containing 'image' and 'label' subfolders.",
    )
    parser.add_argument(
        "target_folder",
        type=str,
        help="Path to the target folder to save resampled MHA files.",
    )
    parser.add_argument(
        "--spacing",
        type=float,
    ...
    parser.add_argument(
        "--size", type=int, nargs="+", default=None, help="Target size for resampling."
    )
    parser.add_argument(
        "--mp", action="store_true", default=False, help="Enable multiprocessing."
    )
    args = parser.parse_args()

    ResampleFolder(
        args.source_folder, args.target_folder, args.spacing, args.size, args.mp
    )
