...
def generate_mha_difference_map(mha1: str, mha2: str, output_path: str):
    ...
    """
    assert (
        mha1.endswith(".mha") and mha2.endswith(".mha") and output_path.endswith(".mha")
    )
    ...
    for value in all_unique_value:
        ...
        # 重叠且不是背景
        one_hot_map[
            (one_hot_source_1 == one_hot_source_2) * one_hot_source_1 * one_hot_source_2
        ] = 1
        # 仅mha1存在
...
def process_a_pair_folder(mha_folder1: str, mha_folder2: str, output_folder: str):
    ...
    mha_files1 = [
        osp.join(mha_folder1, file)
        for file in os.listdir(mha_folder1)
        if file.endswith(".mha")
    ]
    mha_files2 = [
        osp.join(mha_folder2, file)
        for file in os.listdir(mha_folder2)
        if file.endswith(".mha")
    ]
    assert len(mha_files1) == len(mha_files2)

    for mha1, mha2 in tqdm(
        zip(mha_files1, mha_files2),
        desc="Processing",
...
def parse_args():
    ...
    parser.add_argument(
        "mha_folder1",
        type=str,
        help="The first mha folder path, which will be subtracted.",
    )
    parser.add_argument(
        "mha_folder2",
        type=str,
        help="The second mha folder path, which will to be the subtractor.",
    )
