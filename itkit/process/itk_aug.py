...
class AugProcessor(SeparateFoldersProcessor):
    ...
    def random_3d_rotate(
        self, image: sitk.Image, label: sitk.Image, angle_ranges: Sequence[float]
        ...
        radian_angles = [
            np.deg2rad(random.uniform(-angle_range, angle_range))
            for angle_range in angle_ranges
        ][::-1]
        size = image.GetSize()
        spacing = image.GetSpacing()
        origin = image.GetOrigin()
        center_point = [origin[i] + spacing[i] * size[i] / 2.0 for i in range(3)]

        transform = sitk.Euler3DTransform()
        transform.SetCenter(center_point)
        transform.SetRotation(radian_angles[0], radian_angles[1], radian_angles[2])

        rotated_image = sitk.Resample(image, transform, INTERPOLATOR("image"), -3072)
        rotated_label = sitk.Resample(label, transform, INTERPOLATOR("label"), 0)
    ...
    def process_one(self, args: tuple[str, str]) -> list[SeriesMetadata] | None:
        ...
        for i in range(self.aug_num):
            rotated_image, rotated_label = self.random_3d_rotate(
                image, label, self.random_rots
            )
            # save to mha
            ...
            if self.output_folder_B:
                ...
                lbl_meta = SeriesMetadata(
                    ...
                    include_classes=np.unique(
                        sitk.GetArrayFromImage(rotated_label)
                    ).tolist(),
                )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional folder for augmented output image mhas.",
    )
    parser.add_argument(
        "-olbl",
        "--out-lbl-folder",
        type=str,
        default=None,
        help="Optional folder for augmented output label mhas.",
    )
    parser.add_argument(
        "-n",
        "--num",
        type=int,
        default=1,
        help="Number of augmented samples of each source sample.",
    )
    parser.add_argument(
        "--mp",
        action="store_true",
        help="Enable multiprocessing, the number of workers are `None`.",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs=3,
        default=None,
        help="Maximum ramdom rotation degree on `Z Y X` axis.",
    )
