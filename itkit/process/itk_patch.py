...
class ProcessOneResult(BaseModel):
    ...
    patch_metadata_list: list[SeriesMetadata] = Field(
        default_factory=list, description="Metadata for each extracted patch"
    )
    source_metadata: "PatchMetadata" = Field(
        ..., description="Metadata about the source series"
    )
...
class PatchMetadata(BaseModel):
    ...
    shape: tuple[int, int, int] = Field(
        ..., description="Original image shape (Z, Y, X)"
    )
    ...
    class_within_patch: dict[str, list[int]] = Field(
        default_factory=dict, description="Mapping patch filename to unique classes"
    )
...
class CropMetadata(BaseModel):
    ...
    anno_available: list[str] = Field(
        default_factory=list,
        description="List of series IDs with available annotations",
    )
    patch_meta: dict[str, PatchMetadata] = Field(
        default_factory=dict, description="Per-series patch metadata"
    )
...
class PatchProcessor(DatasetProcessor):
            ...
            if res and isinstance(res, ProcessOneResult):
                ...
                for patch_meta in res.patch_metadata_list:
                    self.meta_manager.update(
                        patch_meta,
                        allow_and_overwrite_existed=self.ALLOW_AND_OVERWRITE_EXISTED_METADATA,
                    )

                # Collect source-level metadata for crop_meta.json
                self.source_metadata[res.source_metadata.series_id] = (
                    res.source_metadata
                )
    ...
    def extract_patches(
        ...
        def to_triplet(x):
            ...
            if isinstance(x, (list, tuple)) and len(x) == 3:
                return tuple(x)
            raise ValueError(
                "patch_size and patch_stride must be int or 3-length list/tuple"
            )
                ...
                for x in x_starts:
                    ...
                    if no_label:
                        ...
                        if fg_ratio < minimum_foreground_ratio:
                            save = False
                        if fg_ratio == 0 and (
                            np.random.rand() > self.keep_empty_label_prob
                        ):
                            save = False
    ...
    def process_one(self, args: tuple[str, str]) -> ProcessOneResult | None:
        ...
        try:
            ...
            return ProcessOneResult(
                patch_metadata_list=patch_metadata_list, source_metadata=source_meta
            )
    ...
    def is_valid_sample(self, itk_img: sitk.Image, itk_lbl: sitk.Image) -> bool:
        ...
        if not np.allclose(img_size, lbl_size, atol=1.5):
            tqdm.write(
                f"Skipping for Size mismatch img size {img_size} | lbl size {lbl_size}."
            )
            return False
        ...
        if not np.allclose(img_spacing, lbl_spacing, atol=0.01):
            tqdm.write(
                f"Skipping for Spacing mismatch img spacing {img_spacing} | lbl spacing {lbl_spacing}."
            )
            return False
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract patches from a folder of MHA images"
    )
    parser.add_argument(
        "src_folder", type=Path, help="Folder containing `image` and `label` subfolders"
    )
    parser.add_argument(
        "dst_folder", type=Path, help="Destination root folder to save patches"
    )
    parser.add_argument(
        "--patch-size",
        type=int,
    ...
    parser.add_argument(
        "--mp", action="store_true", help="Use multiprocessing to process cases"
    )
    parser.add_argument(
        "--workers",
        type=int,
