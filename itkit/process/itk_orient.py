...
class OrientProcessor(SingleFolderProcessor):
    def __init__(
        ...
        super().__init__(
            source_folder, dest_folder, recursive=True, mp=mp, workers=workers
        )
    ...
    def process_one(self, file_path: str) -> SeriesMetadata | None:
        ...
        return SeriesMetadata(
            ...
            include_classes=(
                np.unique(sitk.GetArrayFromImage(oriented_img)).tolist()
                if self.field == "label"
                else None
            ),
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert all .mha files under the source directory to the specified orientation (e.g. LPI) while preserving the original directory structure."
    )
    ...
    processor = OrientProcessor(
        args.src_dir, args.dst_dir, args.orient, args.field, args.mp, args.workers
    )
