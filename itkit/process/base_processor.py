...
class SeparateFoldersProcessor(BaseITKProcessor):
    ...
    def get_items_to_process(self) -> list[tuple[str, str]]:
        ...
        files_A = {
            self._normalize_filename(os.path.basename(f)): f for f in files_A_paths
        }
        files_B = {
            self._normalize_filename(os.path.basename(f)): f for f in files_B_paths
        }
...
class DatasetProcessor(BaseITKProcessor):
    ...
    def __init__(
        self, source_folder: str, dest_folder: str | None = None, *args, **kwargs
    ):
        """
    ...
    def get_items_to_process(self) -> list[tuple[str, str]]:
        ...
        if not (os.path.isdir(img_dir) and os.path.isdir(lbl_dir)):
            raise ValueError(
                f"Missing 'image' or 'label' subfolders in {self.source_folder}"
            )
    ...
    def start_from_arg(cls):
        ...
        argparser.add_argument(
            "source_folder",
            type=str,
            help="Root folder containing 'image' and 'label' subfolders",
        )
        argparser.add_argument(
            "--dest-folder",
            type=str,
        ...
        argparser.add_argument(
            "--mp", action="store_true", help="Enable multiprocessing"
        )
        argparser.add_argument(
        ...
        return cls(
            args.source_folder, args.dest_folder, args.mp, args.workers, args.recursive
        )
