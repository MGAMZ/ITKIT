...
class SeriesMetadata(BaseModel):
    ...
    spacing: tuple[float, float, float] = Field(
        ..., description="Image spacing (Z, Y, X)"
    )
    size: tuple[int, int, int] = Field(..., description="Image size (Z, Y, X)")
    origin: tuple[float, float, float] = Field(
        ..., description="Image origin (Z, Y, X)"
    )
    include_classes: tuple[int, ...] | None = Field(
        None, description="Classes to include in processing"
    )
    ...
    def validate_itk_image(self, image: sitk.Image) -> bool:
        ...
        if not np.allclose(img_spacing_zyx, self.spacing, rtol=1e-5):
            raise ValueError(
                f"Spacing mismatch: expected {self.spacing}, got {img_spacing_zyx}"
            )
        ...
        if not np.allclose(img_origin_zyx, self.origin, rtol=1e-5):
            raise ValueError(
                f"Origin mismatch: expected {self.origin}, got {img_origin_zyx}"
            )
...
class MetadataManager:
    ...
    def update(
        self, image_meta: SeriesMetadata, allow_and_overwrite_existed: bool = True
    ):
        if (image_meta.name not in self.meta) or allow_and_overwrite_existed:
            ...
            raise ValueError(
                f"Metadata for {image_meta.name} already exists and differs.\n"
