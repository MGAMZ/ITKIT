...
class _ResampleMixin:
    ...
    def resample_one_sample(
        self, input_path: str, field: Literal["image", "label"], output_path: str
    ) -> None | SeriesMetadata:
        ...
        output_path = (
            output_path.replace(".nii.gz", ".mha")
            .replace(".nii", ".mha")
            .replace(".mhd", ".mha")
        )
        ...
        if self.resampling_mode == ResamplingMode.TARGET_IMAGE:
            image_resampled = self._resample_to_target_image(
                image_itk, input_path, field
            )
        ...
        return SeriesMetadata(
            ...
            include_classes=(
                np.unique(sitk.GetArrayFromImage(image_resampled)).tolist()
                if field == "label"
                else None
            ),
        )
    ...
    def _get_target_path(
        self, input_path: str, field: Literal["image", "label"]
    ) -> str | None:
    ...
    def _resample_to_target_image(
        self, image_itk: sitk.Image, input_path: str, field: Literal["image", "label"]
    ) -> sitk.Image | None:
    ...
    def _apply_spacing_size_rules(
        self, image_itk: sitk.Image, field: Literal["image", "label"]
    ) -> sitk.Image:
        ...
        if needs_spacing_resample and not np.allclose(effective_spacing, orig_spacing):
            image_after_spacing = sitk_resample_to_spacing(
                image_itk, effective_spacing, field
            )
        assert isinstance(
            image_after_spacing, sitk.Image
        ), "Resampling failed, result is not a SimpleITK image."
        ...
        if needs_size_resample and effective_size != list(current_size):
            image_resampled = sitk_resample_to_size(
                image_after_spacing, effective_size, field
            )
...
class ResampleProcessor(DatasetProcessor, _ResampleMixin):
    ...
    def _get_target_path(
        ...
        assert (
            self.resampling_mode == ResamplingMode.TARGET_IMAGE
        ), "Target path requested in non-target-image mode."
        assert (
            self.target_folder is not None
        ), "Target folder must be specified for target image mode."
    ...
    def process_one(self, args) -> None | SeriesMetadata:
        ...
        img_out_path = os.path.join(
            self.dest_folder, "image", os.path.basename(img_path)
        )
        lbl_out_path = os.path.join(
            self.dest_folder, "label", os.path.basename(lbl_path)
        )

        if Path(img_out_path).exists() and Path(lbl_out_path).exists():
            print(
                f"Output files already exist, skipping: {img_out_path}, {lbl_out_path}"
            )
            return None

        img_meta = self.resample_one_sample(
            input_path=img_path, field="image", output_path=img_out_path
        )
        lbl_meta = self.resample_one_sample(
            input_path=lbl_path, field="label", output_path=lbl_out_path
        )
        return lbl_meta
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Resample a dataset with dimension-wise spacing/size rules or target image."
    )
    parser.add_argument(
        "mode",
        type=str,
        choices=["image", "label", "dataset"],
        help="Resample mode: single-folder 'image'/'label' or paired 'dataset'.",
    )
    parser.add_argument(
        "source_folder",
        type=str,
        help="The source folder. For 'dataset' mode, it must contain 'image' and 'label' subfolders.",
    )
    parser.add_argument(
        "dest_folder",
        type=str,
        help="The destination folder. For 'dataset' mode, outputs to 'image' and 'label' subfolders.",
    )
    parser.add_argument(
        "-r",
        "--recursive",
    ...
    parser.add_argument(
        "--mp", action="store_true", help="Whether to use multiprocessing."
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def validate_and_prepare_args(args):
    ...
    if target_specified:
        ...
        if len(target_spacing) != 3:
            raise ValueError(
                f"--spacing must have 3 values (received {len(target_spacing)})"
            )
        ...
        for i in range(3):
            if target_spacing[i] != -1 and target_size[i] != -1:
                raise ValueError(
                    f"Cannot specify both spacing and size for dimension {i}."
                )
    ...
    print(
        f"  Recursive: {args.recursive} | Multiprocessing: {args.mp} | Workers: {args.workers}"
    )
...
def main():
    ...
    processor.process()
    print(
        f"Resampling completed. The resampled dataset is saved in {args.dest_folder}."
    )
